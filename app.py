...
new_user_info = {
    ...
    'Rank (out of 100)': [user_info['user_rank_1m'], user_info['user_rank_1h'], user_info['user_rank_1d'], user_info['user_rank_1w']]
...
df['Rank (out of 100)'] = df['Rank (out of 100)'].astype(int)


image = Image.open('musk.jpg')
...
if username == 'elonmusk':
    ...
    with cola:
        ...
        st.write(df.style.applymap(highlight_less_than_5, subset=['Rank (out of 100)']).applymap(highlight_less_than_0, subset=['Average Profit']))
        st.write('<h3>Recommendations:</h3>', unsafe_allow_html=True)
        st.write('<h7><b>1-Minute Trading:</b> For short-term trading insights, consider following Twitter users such as <b>@zerohedge</b> and <b>@mikealfred</b></h7>', unsafe_allow_html=True)
        st.write('<h7><b>1-Hour Trading:</b> For a 1-hour trading perspective, <b>@elonmusk</b> is highly recommended</h7>', unsafe_allow_html=True)
        st.write('<h7><b>1-Day Trading:</b> For daily trading insights, we recommend following <b>@elonmusk</b> and <b>@canuck2usa</b> </h7>', unsafe_allow_html=True)
        st.write('<h7><b>1-Week Trading:</b> While <b>@elonmusk</b> remains a top performer, our analysis shows that following <b>@Fxhedgers</b> may yield better results.</h7>', unsafe_allow_html=True)
    ...
    col1_content = "<h3 style='text-align: center;'>Experts better at 1 Min predictions</h3>"
